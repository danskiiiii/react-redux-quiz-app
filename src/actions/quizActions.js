...
export const onNextClick = () => {
  ...
  if (store.getState().quiz.answeredBlocks <6){
    return ({type: NEXT,  payload:{                         
      currentQuestion : store.getState().quiz.questionsOrder[store.getState().quiz.answeredBlocks+1],
    ...
    return ({type: NEXT , payload:{             
      ...
      isResultShown : true,
      percentResult: (store.getState().quiz.totalPoints/((store.getState().quiz.answeredBlocks+1)*3)*100).toFixed(2)}})
  } 
};  
